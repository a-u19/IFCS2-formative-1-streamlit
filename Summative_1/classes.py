...
class Manager(Employee):
    ...
    def add_staff(self, new_staff_id: int, first_name: str, last_name: str, manager_id:int) -> None:
        """
        ...
        if new_staff_id not in self.staff_list:
            ...
            new_staff = Staff(new_staff_id, first_name, last_name, manager_id)
    ...
    def remove_staff(self, staff_id: int) -> None:
        ...
        if staff_id in self.staff_list:
            ...
            staff_data = handle_csv('data/staff_details.csv', 'r')
            updated_data = [row for row in staff_data if row['staff_id'] != str(staff_id)]

            if updated_data:
                handle_csv('data/staff_details.csv', 'w', updated_data, list(updated_data[0].keys()))
            else:
                # If no data left, write empty file with headers
                handle_csv('data/staff_details.csv', 'w', [], ['staff_id', 'first_name', 'last_name'])
    ...
    def edit_staff_name(self, staff_id: int, new_first_name: str, new_last_name: str) -> None:
        ...
        if updated:
            handle_csv('data/staff_details.csv', 'w', staff_data, list(staff_data[0].keys()))
...
class Staff(Employee):
    ...
    def see_call_history(self) -> None:
        ...
        """
        call_data = handle_csv('data\\call_details.csv', 'r')
